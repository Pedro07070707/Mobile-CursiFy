...
const adminService = {
  getAll: async (): Promise<AdminOverview> => {
    ...
    return {
      ...
      students_total: users.filter((user: BackendUser) => user.nivelAcesso === "ALUNO").length,
      teachers_total: users.filter((user: BackendUser) => user.nivelAcesso === "PROFESSOR").length,
      admins_total: users.filter((user: BackendUser) => user.nivelAcesso === "ADMIN").length,
      courses_total: coursesResponse.data.length,
      enrollments_total: 0,
    };
  },

  getUsers: () => api.get<BackendUser[]>("/usuario").then((response: { data: BackendUser[] }) => response.data.map(mapUser)),
